...
from data import data_char2_rot
...
def train(opt):
    ...
    print(f"训练集的数量:{len(data_char2_rot.datasets_train)}")
    print(f"验证集的数量:{len(data_char2_rot.datasets_val)}")
    ...
    for epoch in range(start_epoch, epoch_count):
        ...
        for imgs, labels in data_char2_rot.dataloader_train:
            imgs = imgs.to(device)
        ...
        with torch.no_grad():
            for imgs, labels in data_char2_rot.dataloader_val:
                imgs = imgs.to(device)
        ...
        train_acc = total_train_accuracy / len(data_char2_rot.datasets_train)
        train_loss = total_train_loss
        val_acc = total_val_accuracy / len(data_char2_rot.datasets_val)
